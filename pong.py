...
def aftermatch(text):
    ...
    while run:
        ...
        btn_rematch = create_button('REMATCH',screen_width/2 - 110,screen_height/2 + 15,220,55)
        btn_mainmenu = create_button('CANCEL',screen_width/2 - 95,screen_height/2 + 100,175,55)
...
def onlinematch():
    global screen,game_font,screen_width,screen_height
    run = True
    try:
        n = Network()
        player = int(n.getP())
        ...
        p = n.send("player")
        player_font = pygame.font.SysFont("Striger.ttf", 65)
        ...
        while run:
            ...
            if not(game.connected()):
                ...
                if current_time - start_time>=(pause_time+15000):
                    n.close()
                    run= False
                ...
                if current_time - start_time >= pause_time:
                    b = n.send("ball")
                ...
                text_color = (color_r,color_b,color_g)
                player_text = player_font.render("YOU",False,text_color)

                if player == 0:
                    screen.blit(player_text,(screen_width/2 - 200,screen_height/2))
                elif player == 1:
                    screen.blit(player_text,(screen_width/2 + 175,screen_height/2))
